...
The yaz-client command is able to connect to and search through a library database. It is not a line search system like grep, but instaed performs 739.50/SRU searches, which has some overlap with Boolean searches (although yaz formats queries differently).
